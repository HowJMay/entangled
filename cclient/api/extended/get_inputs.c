...
retcode_t iota_client_get_inputs(iota_client_service_t const* const serv, flex_trit_t const* const seed,
  ...
  hash243_queue_entry_t* addresses_q_iter = NULL;
  // get address list
  ret_code = iota_client_get_new_address(serv, seed, addr_opt, &addresses);
  if (ret_code) {
    log_error(client_extended_logger_id, "%s get new address failed: %s\n", __func__, error_2_string(ret_code));
    goto done;
  }

  for (uint64_t addr_index = addr_opt.start; addr_index < addr_opt.total;
       (addr_index + 5 < addr_opt.total) ? (addr_index += 5) : (addr_opt.total - 1)) {
    ...
    uint64_t addr_push_index_end = addr_index + 5;
    flex_trit_t* addr;
    for (uint64_t addr_push_index = addr_index; addr_push_index < addr_push_index_end; addr_push_index++) {
      if (addr = hash243_queue_at(addresses, addr_push_index) != NULL) {
        hash243_queue_push(balances_req->addresses, addr);
      } else {
        break;
      }
    }
